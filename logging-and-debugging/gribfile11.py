...
# The class ``closing`` implements a context manager.
class closing(object):
    ...
    def __enter__(self):
        # The method ``__enter__`` is called before Python enters the context
        # block
        self.log.debug("closing(): Entering context")
        return self.file_object

    def __exit__(self, *exc_info):
        # The method __exit__ is called befor Python leaves the context block,
        # even if errors where raised within the context block
        self.log.debug("closing(): Errors in context: %s", exc_info)
        self.log.debug("closing(): Closing file")
        self.file_object.close()
        self.log.debug("closing(): Leaving context")
